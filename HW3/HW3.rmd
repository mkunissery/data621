...
train <- read.csv('https://raw.githubusercontent.com/mkunissery/Data621/master/HW3/crime-training-data_modified.csv', header=T)
test <- read.csv('https://raw.githubusercontent.com/mkunissery/Data621/master/HW3/crime-evaluation-data_modified.csv', header=T)
...
Crime has existed as long as humans have been. Various factors in combination such as deep differences in economic strata, pyschological disturbances arising out of intoxicating products , lack of culture in raising kids and multitude of other factors plays a key role. The attached spreadsheet(csv) files lists out a set of predictor variables though not outlined above and here i am attempting to create a relationship using various  models that predict crime rate .

## Statement of the Problem

This report presents a statistical model aimed at determining variables that are independently associated with crime rates above or below the median.
...
download.file("https://github.com/mkunissery/Data621/blob/master/HW3/Data%20Dictionary.xlsx?raw=true", destfile = "DataDict.xlsx", mode = 'wb')
daDict <- read.xlsx("DataDict.xlsx", sheetIndex = 1)
file.remove(dir(getwd(), pattern = "DataDict.xlsx", full.names = TRUE))
dfDict <- data.frame(matrix(unlist(daDict), nrow=14, byrow=T),stringsAsFactors=FALSE)
```

### Check distribution 

Check distribution of the target variable in our training data.
...
## 2. DATA CLEANSING
 ...
 We can see from our visualizations a few variables with some issues.   We'll modify `rad` so that the value for 24 is now sequential and 9.  We'll also center and scale our data based on the mean and standard deviation of each variable during the model building step. We'll otherwise avoid binning. 
...
A review of the VIF (Variance Inflation Factors) output of the model suggests some points that are highly colinear and a number of variables that may not be necessary.   Model 1 uses the formula:
...
m4cM <- confusionMatrix(preds4, validation$target, 
                        mode = "everything")
par(mfrow=c(2,2))
fourfoldplot(m1cM$table, color = c("#B22222", "#8b2e67"), main="Model 1")
fourfoldplot(m2cM$table, color = c("#B22222", "#8b2e67"), main="Mod2")
fourfoldplot(m3cM$table, color = c("#B22222", "#8b2e67"), main="Mod3")
fourfoldplot(m4cM$table, color = c("#B22222", "#8b2e67"), main="Mod4")
```
...
write.csv(finaldf, 'Predictions.csv', row.names = FALSE)
